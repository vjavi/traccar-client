<template>
  <div class="h-screen flex flex-col bg-dark-200">
    <!-- Header -->
    <header class="bg-dark-100/90 backdrop-blur-lg border-b border-white/10 px-6 py-4 flex items-center justify-between z-50">
      <div class="flex items-center gap-4">
        <div class="flex items-center gap-3">
          <div class="w-10 h-10 bg-gradient-to-br from-traccar-400 to-traccar-600 rounded-xl flex items-center justify-center shadow-lg shadow-traccar-500/20">
          ...
          <div>
            <h1 class="text-lg font-display font-bold text-white">Traccar Client</h1>
            <p class="text-xs text-slate-400">{{ authStore.traccarUrl }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="flex items-center gap-4">
        ...
        <button 
          @click="refreshData" 
          :disabled="loading"
          class="p-2.5 bg-dark-200/50 hover:bg-dark-200 rounded-xl text-slate-400 hover:text-white transition-all"
    ...
    <div class="flex-1 flex overflow-hidden">
      <!-- Sidebar -->
      <aside class="w-80 bg-dark-100/50 backdrop-blur-lg border-r border-white/10 flex flex-col">
        <!-- Tabs -->
      ...
      <main class="flex-1 relative">
        ...
        <div 
          ...
          class="absolute top-4 right-4 w-80 bg-dark-100/95 backdrop-blur-xl rounded-2xl border border-white/10 shadow-2xl animate-slide-in overflow-hidden"
        >
          <div class="p-4 border-b border-white/10 flex items-center justify-between">
            <h3 class="font-display font-semibold text-white">{{ selectedDeviceInfo.device.name }}</h3>
